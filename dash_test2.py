...
# Plotly graph printing called from print_out
def plot_dat(file_dat, patient):
    ...
    for key, items in file_dat.items():
        if key != 'Time':
            fig.add_trace(go.Scatter(x=file_dat['Time'][0::4], y=file_dat[key][0::4], name='Unedited ' + key,
                                     line=dict(color='rgb(251,180,174)')), row=plots, col=1)
            fig.add_trace(go.Scatter(x=file_dat['Time'][0::4], y=remove_baseline_wander(file_dat[key], 2000.0)[0::4],
                                     name='Corrected ' + key, line=dict(color='royalblue')), row=plots, col=1)
            file_dat[key].clear()
...
# -- Dash Layout --
# Build main interface for callbacks to update as necessary

external_stylesheets = ['https://codepen.io/chriddyp/pen/bWLwgP.css']

# Initialize app, called as main
...
app.layout = html.Div(className='row', children=[
        ...
        html.Div([
            dcc.Upload(
                id='upload-data',
                    children=html.Div([
                    'Upload data (.csv and .hdr):  Drag and Drop or ',
                    html.A('Select Files')
            ]),
            style={
                'width': '100%',
    ...
    dcc.Textarea(
        id = 'notes',
        value = 'Upload data using the red box.',
...
def update_notes(list_of_names):
    count = 0
    value = 'Upload the .hdr and .csv files for a single data set using the red box.'
...
def update_data(list_of_contents, list_of_names):
    ...
    if count ==2 and dat_name == hdr_name:
        ...
        #Call file opener, returns tuple of the hdr & csv lists, then unpacks
        ...
        fig = plot_dat(final_dat, patient)
