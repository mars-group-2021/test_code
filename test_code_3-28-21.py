...
def negdata_flip(data_section):
    # invert any raw negative mV peaks to positive, normal ECG
    # input:  data       = section to be evaluated (1d list or numpy array)
    # output: out_array  = 1d array

    #out_array = np.array(data_section)
...
def tall_peak_reduct(lst):
    ...
    samp_start = None  #placeholder for the starting index of sample selection
    samp_end = None  #placeholder for the ending index of sample selection
    inv_guesses = []  #list of guesses for each selection, whether selection is beleived to be inverted, normal, or unknown
    ...
    for i, v in enumerate(lst):
        if i > 0:
            ...
            if samp_start == None and slope <= not_flat: #starts similarly to the first part, after slope is not flat
                samp_start = i-1  # index of the start of the sample selection
                ss_v = v  # value of the start of the sample selection
            if samp_start != None and (slope >= steep_slope or abs(v-ss_v) > scale_max):  #if sample selection has started and either a steep slope or tall peak is detected
                samp_start = None  # restart selection
            if samp_start != None and i-samp_start == 1000: #once clean selection of 1000 data points is found
                samp_end = i  # end index of sample selection
                samp_vals = lst[samp_start:samp_end] #list of values from start to end of sample
                max_sv = max(samp_vals) # max value of sample
                min_sv = min(samp_vals) # min value of sample
                mid_sv = (max_sv + min_sv)/2 # midpoint of sample
                top = sum(1 for x in samp_vals if mid_sv <= x <= max_sv)/len(samp_vals)*100 # percentage of values within the sample selection that fit in the top half
                bottom = sum(1 for x in samp_vals if min_sv <= x <= mid_sv)/len(samp_vals)*100 # percentage of values within the sample selection that fit in the bottom half
                if top >= 75: # if 75% or more of the data points are in the top half of the sample range
                    inv_guesses.append('inv') #add guess for this sample as 'inverted'
                elif bottom >= 75: # if 75% or more of the data points are in the bottom half of the sample range
                    inv_guesses.append('norm') #add guess for this sample as 'normal'
                else:
                    inv_guesses.append('unk') #add guess for this sample as 'unknown'
                samp_start = None #restart selection

    
    if len(inv_guesses) > 0:
        guess = max(inv_guesses,key=inv_guesses.count) #best guess based on highest occurance of guesses
        guess_perc = round(inv_guesses.count(guess)/len(inv_guesses)*100,2) #probability of best guess
        if guess == 'inv' and guess_perc > 60: # if best guess is 'inverted' and probability is higher than 60%
            print(' Inverted signal detected')
            print(' Un-inverting data')
            lst[:] = negdata_flip(lst) #flip signal
...
def plot_out(final_dat, patient):
    ...
    print('    flat/steep peak reduction, and signal inversion (if needed)')
    while True:
